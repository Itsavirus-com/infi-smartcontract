import { ClaimGateway__factory } from '@project/contracts/typechain';
...
type DeployArgs = Parameters<ClaimGateway__factory['deploy']>;

const NAME = 'ClaimGateway';
const HELPER_NAME = 'ClaimHelper';
...
const func: DeployFunction = async function ({
  ...
  await deployments.deploy(HELPER_NAME, {
    // This will create a deployment called 'Market'. By default it will look for an artifact with the same name. The 'contract' option allows you to use a different artifact.
    from: deployerAddress, // Deployer will be performing the deployment transaction.
    args, // Arguments to thecontract's constructor.
    log: true, // Display the address and gas used in the console (not when run in test though).
...
func.tags = ['ClaimGateway']; // This sets up a tag so you can execute the script on its own (and its dependencies).
