...
describe('Create Request until Claim', () => {
   ...
   * - Funder 1 Take Back Deposit, Failed
  ...
  describe('Flow 1 ', async () => {
    ...
    it('Funder 1 Take Back Deposit, Failed', async () => {
      ...
      await expect(
        claimGateway.connect(funder1).refundDepositOfProvideCover(0)
    ...
    it('Validate Pending Claims', async () => {
      ...
      await setUpMockKeepers(
        ...
        1627886940,
        { past: 740000, current: 740000, next: 740000 }
      );

      const requestId = 0;
      await claimGateway.validatePendingClaims(LISTING_TYPE.REQUEST, requestId);
    });
