...
describe('Create Request until Claim', () => {
  ...
  describe('Flow 3 ', async () => {
    ...
    it('Funder 1 Take Back Deposit Collectively', async () => {
      ...
      const payout1 = ethers.utils
        .parseUnits('1000', usdtDecimal)
        .mul(devaluation)
        .div(currentPrice);
      const depositLeft1 = ethers.utils
        .parseUnits('1000', usdtDecimal)
        .sub(payout1);
      const depositLeft2 = ethers.utils.parseUnits('3000', usdtDecimal);
      const depositLeft = depositLeft1.add(depositLeft2);
