...
describe('Create Offer until Claim, Valid Claims 3', () => {
   ...
   * Note
   * use block : 17077993
   * Precondition : checkClaimForDevaluation must return (true, 750000, 6)
   * Scenario
  ...
  describe('Flow 3', async () => {
    ...
    it('Holder 1 Make Claim', async () => {
      // Set Mockup Keepers
      await setUpMockKeepers(['18446744073709555607'], 1627824150, {
        past: 740000,
        current: 740000,
        next: 740000,
      });
      ...
      // Time Travel
      await network.provider.send('evm_setNextBlockTimestamp', [1628086950]);
    ...
    it('Holder 2 Make Claim , No Payout', async () => {
      // Set Mockup Keepers
      await setUpMockKeepers(['18446744073709555936'], 1627864150, {
        past: 740000,
        current: 740000,
        next: 740000,
      });
    ...
    it('Payout Dev Wallet', async () => {
      ...
      const expiredPayout = ethers.utils
        .parseUnits('1000', usdtDecimal)
        .mul(devaluation)
        .div(currentPrice);
      const devTokenAfter = await usdtToken.balanceOf(devWallet.address);
      expect(devTokenAfter).to.eq(devTokenBefore.add(expiredPayout));
    });
  });
});
